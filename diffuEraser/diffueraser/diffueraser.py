...
def read_mask(validation_mask, fps, n_total_frames, img_size, mask_dilation_iter, frames):
    ...
    if os.path.isdir(validation_mask):
        ...
        mask_files = sorted([f for f in os.listdir(validation_mask) if f.endswith(('.png', '.jpg', '.jpeg'))])
        for idx, mask_file in enumerate(mask_files[:n_total_frames]):
            mask_path = os.path.join(validation_mask, mask_file)
...
def read_priori(priori, fps, n_total_frames, img_size, keep_priori=False):
    prioris = []
    
    # Check if input is a directory or video file
    if os.path.isdir(priori):
        # Read priori frames from directory
        priori_files = sorted([f for f in os.listdir(priori) if f.endswith(('.png', '.jpg', '.jpeg'))])
        for priori_file in priori_files[:n_total_frames]:
            priori_path = os.path.join(priori, priori_file)
            img = Image.open(priori_path).convert('RGB')
...
def read_video(validation_image, video_length, nframes, max_img_size, fps=None):
    # Check if input is a directory or video file
    if os.path.isdir(validation_image):
        # Read frames from directory - read ALL frames, ignore video_length
        frame_files = sorted([f for f in os.listdir(validation_image) if f.endswith(('.png', '.jpg', '.jpeg'))])
        frames = []
        for frame_file in frame_files:
            frame_path = os.path.join(validation_image, frame_file)
            frame = Image.open(frame_path).convert('RGB')
            frames.append(frame)
        
        # Use provided fps or default to 12
        if fps is None:
            fps = 12
...
class DiffuEraser:
    def __init__(
        ...
        if loaded != (ckpt + mode):
            # Construct PCM_Weights path from diffueraser_path
            # diffueraser_path is like /path/to/weights/diffuEraser
            # we need /path/to/weights/PCM_Weights
            import os
            weights_dir = os.path.dirname(diffueraser_path)
            pcm_weights_path = os.path.join(weights_dir, "PCM_Weights")
    ...
    def forward(self, validation_image, validation_mask, priori, output_path,
                ...
                nframes=22, seed=None, revision = None, guidance_scale=None, blended=True, output_as_frames=False, fps=None):
        validation_prompt = ""  # 
        ...
        ################ read input video ################ 
        frames, fps, img_size, n_clip, n_total_frames = read_video(validation_image, video_length, nframes, max_img_size, fps=fps)
        ...
        if output_as_frames:
            ...
            for f in range(real_video_length):
                ...
                frame_path = os.path.join(output_path, f"{f:05d}.png")
