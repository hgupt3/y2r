...
size_t size_function__PredictedTracks__trajectory_z(const void * untyped_member)
{
  const auto * member = reinterpret_cast<const std::vector<float> *>(untyped_member);
  return member->size();
}

const void * get_const_function__PredictedTracks__trajectory_z(const void * untyped_member, size_t index)
{
  const auto & member =
    *reinterpret_cast<const std::vector<float> *>(untyped_member);
  return &member[index];
...
void * get_function__PredictedTracks__trajectory_z(void * untyped_member, size_t index)
{
  auto & member =
    *reinterpret_cast<std::vector<float> *>(untyped_member);
  return &member[index];
...
void fetch_function__PredictedTracks__trajectory_z(
  const void * untyped_member, size_t index, void * untyped_value)
{
  const auto & item = *reinterpret_cast<const float *>(
    get_const_function__PredictedTracks__trajectory_z(untyped_member, index));
  auto & value = *reinterpret_cast<float *>(untyped_value);
  value = item;
}

void assign_function__PredictedTracks__trajectory_z(
  void * untyped_member, size_t index, const void * untyped_value)
{
  auto & item = *reinterpret_cast<float *>(
    get_function__PredictedTracks__trajectory_z(untyped_member, index));
  const auto & value = *reinterpret_cast<const float *>(untyped_value);
  item = value;
}

void resize_function__PredictedTracks__trajectory_z(void * untyped_member, size_t size)
{
  auto * member =
    reinterpret_cast<std::vector<float> *>(untyped_member);
  member->resize(size);
...
static const ::rosidl_typesupport_introspection_cpp::MessageMember PredictedTracks_message_member_array[6] = {
  ...
  },
  {
    ...
    0,  // array size
    false,  // is upper bound
    offsetof(tracker_interfaces::msg::PredictedTracks, trajectory_z),  // bytes offset in struct
    nullptr,  // default value
    size_function__PredictedTracks__trajectory_z,  // size() function pointer
    get_const_function__PredictedTracks__trajectory_z,  // get_const(index) function pointer
    get_function__PredictedTracks__trajectory_z,  // get(index) function pointer
    fetch_function__PredictedTracks__trajectory_z,  // fetch(index, &value) function pointer
    assign_function__PredictedTracks__trajectory_z,  // assign(index, value) function pointer
    resize_function__PredictedTracks__trajectory_z  // resize(index) function pointer
  },
  {
    "num_points",  // name
